...
Token budget matters. Skip narration — don't explain what you're about to do, just do it. No filler text in tool calls or between them.

# COMMIT

Make one or more commits on `{{BRANCH}}` with conventional commit messages. Short: a one-line summary; a body only if a reviewer genuinely can't infer the "why" from the diff, and even then 1-3 lines.
